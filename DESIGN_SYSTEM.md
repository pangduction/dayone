...
| Input (text field)                   | 12 / 10           | —    | 8 (`radius.sm`)  | white bg, `border` 1px, `shadows.xs`, placeholder in `colors.textPlaceholder`. Its 43-tall single-line box is exactly 10 + one 23pt Body line + 10 — built *around* Body's own lineHeight, unlike the search field's fixed-height workaround, but it still hits that same field's centering bug: including `lineHeight` in the style visibly shifts the text off-centre in iOS despite the padding math working out, so the single-line box lists `fontFamily`/`fontSize`/`letterSpacing` only, the same workaround `PostSearchScreen.tsx` already uses. A `multiline` variant (Help & Support's "Contents") swaps in a fixed 240 height instead of `minHeight` — a multiline RN `TextInput` otherwise grows to fit its own content, which drifted from Figma's static box as soon as typing wrapped past a few lines; content past 240 scrolls inside the field instead — and keeps `lineHeight`, since its top-aligned text doesn't hit the centering bug and multiple wrapped lines need it for spacing. |
...
| Header/Add · Header/List (centre)    | 5 (left) / 16 (right) · 16 vertical | 2 (Date Information) | — | back button left, action(s) right, and a "Date Information" block absolutely centred: the date in `typography.caption`/`colors.textPrimary` over a second line in **`typography.overline`**/`colors.textPlaceholder` — the weekday on Header/Add (node `3184:5701`), the post count on Header/List (node `3192:9263`). The two headers differ in their **first** line too: Header/Add names the day being written, Header/List names the month being listed — "August, 2026" (node `3192:9483`), with a comma no `toLocaleDateString` option set produces. The second line was Caption until Figma changed it; only the first line is Caption now. The "Date Information" block's own gap was updated in Figma from 0 to 2 (`spacing[1]`) on both nodes — ported to `AddScreen.tsx`'s `dateInfo` style and `HomeListScreen.tsx`'s `headerCentre` style. |
...
| toggle                                | —                 | —    | 999 (`radius.full`) | Flow 7.1 Notification's switch (component node `9:6902` off / `9:6905` on): 40x18 track, a 14x14 white thumb inset 2 on every side. Off fill `colors.toggleTrackOff` (`#EFEFEF` — close to but distinct from the G-scale, and not a bound variable); on fill `colors.toggleOn` (`#00E585`, a green outside the app's usual accent blue — Figma's own choice for this one control, verified via `get_design_context`, not a mistake to normalize to accent). The thumb's drop shadow flips side with the thumb (`shadows.toggleThumbOff`/`On`); the track's own inset shadow isn't reproduced (RN has no inset shadow, and it isn't visually load-bearing at this size). See `ToggleSwitch.tsx`. |
| Menu (toggle row)                     | (caller's)        | 8    | —            | Flow 7.1's preference row: a two-line label (title `typography.subtext`/`colors.textStrong` over a subtitle `typography.overline`/`colors.textTertiary`, gap 3) and a `toggle`, `flex:1`/`minHeight:44`. Renders its own content only — Figma reuses the identical row shape both standalone (px16/py12, e.g. "Notifications"/"Monthly Report", node `3198:7563`) and nested with no padding of its own inside the expanded Daily Reminder block (node `3199:8123`), so the padding is the caller's job, not this component's. See `NotificationToggleRow.tsx`. |
| Modal/Timer                           | 16 (content)      | —    | 24 (`radius.xl`) | Flow 7.1's time picker, opened by turning Daily Reminder on (or tapping its Timer row). The one dark-themed modal in the app: sheet `colors.timerModalBg` (G900) instead of every other modal's white, otherwise the same slide-up-from-bottom shell technique as `ModalSheet.tsx` (built separately, not as a `dark` prop on it, since the content/actions padding genuinely differ — no content top padding, a plain 16-all-around actions panel). Three independent scrolling wheels (hour 1–12, minute 00–59, AM/PM), 56px rows (Figma-exact) with 3 visible — a real `snapToInterval` `ScrollView` per column, not Figma's static three-row "previous/current/next" mock. The selected row is `Inter_600SemiBold` white; the others `Inter_500Medium` `colors.textSecondary` (G600, matching Figma's dimmed rows exactly). Each row is also its own `Pressable`, not just draggable — tapping a dimmed neighbor (an "8" next to the bold "9") scrolls it to center and commits the value directly, rather than relying on `onMomentumScrollEnd` firing after a *programmatic* animated scroll (unreliable on Android). Figma's own title text reads "Select Month" — a copy-paste leftover from reusing the month-picker modal's header with the wrong text, corrected here to "Select Time" the same way "alomost" was corrected on the Generating-PDF modal. Node `3199:7894`. See `TimerPickerModal.tsx`. |
...
- `src/data/` — mostly local, on-device data stores. `posts.ts`
  ...
  the Export to PDF product rule below. `notificationSettings.ts` is the
...
- `src/pdf/` — `postPageTemplate.ts` holds the shared page size
  ...
  directly from already-captured page JPEGs — no HTML, no WebView, no
  `expo-print` (removed from the project entirely) — see the Export to PDF
  product rule below for why.
...
- **Export to PDF is a real device feature**, not a mock list, per explicit
  ...
  placeholder row. The three behaviors a static Figma mock can't show were
  product decisions rather than reads:
  - **Entry flow**: tapping "Export to PDF" on Setting-Main pushes
    `ExportToPdfScreen` directly, pre-filled with the last 7 days — a
    deliberate deviation from Figma's own node `3199:8735`, which draws the
    date-range modal (`DateRangeModal`) opening *directly over* Setting-Main
    first (its backdrop is the blurred Setting-Main screen itself, confirmed
    by re-examining Figma, not guessed — that part of the read still stands).
    In practice, landing in that modal on every visit buried the screen's own
    "Files" list of already-generated PDFs behind an extra step a user could
    dismiss without noticing — it read as the app having lost track of
    earlier exports, per explicit product feedback. `ExportToPdfScreen`'s own
    "Date Range" row still reopens the identical `DateRangeModal` to change
    the range, so nothing from the Figma flow is missing — only which screen
    is first.
  - **Default range**: the default range, now set as soon as
  ...
  - **The printed file and the in-app preview are the same bytes, read
    ...
    `react-native-view-shot` as a real temp JPEG file — the same
    off-screen-render-and-capture pattern `ShareableCalendarCard` already
    uses for Home's Share button, just to a real file instead of straight to
    the OS share sheet. Those exact bytes become **both** a page of the real
    PDF (`buildPdfFromJpegPages` in `buildPdf.ts` — embedded directly, no
    intermediate rendering step) **and**, moved into permanent storage, the
    file `PdfPreviewScreen` shows (`saveExportFile`'s `pageUris`, alongside
    the PDF itself under `documentDirectory/exports/`). Since the preview
    displays those exact files rather than re-rendering the posts, there is
    no second implementation left that could ever drift from what the file
    actually contains.

    Getting here took two real fixes, not one:
    - **The capture race.** A page captured before its content had actually
      settled could miss a photo's real size, or a `RichTextEditor` (a
      WebView, which renders on its own native surface rather than
      synchronously with the rest of the tree — mounting it isn't the same
      as it having painted) entirely. `PostDetailBody`'s readiness tracking
      waits on *both* `PhotoSection`'s `onLoadSettled` (itself gated on
      `Image.getSize` resolving **and** the `Image`'s own `onLoadEnd`, not
      just `getSize` returning — that alone fires before the resulting
      `aspectRatio` re-render has committed) and `RichTextEditor`'s
      `onLoadEnd`, before a page reports ready. `ExportToPdfScreen.tsx` also
      waits two animation frames after every page reports ready (one for
      that last render to commit, one to paint), and pins `captureRef` to
      the exact print page's pixel dimensions (`PAGE_WIDTH`/`PAGE_HEIGHT`)
      rather than the view's native device-pixel-ratio-scaled resolution.
    - **`expo-print` itself, which this project no longer uses.** Even with
      a correctly-captured screenshot, handing it to `expo-print`'s
      `printToFileAsync` meant a platform WebView decided how to lay the
      image out on the actual PDF page — and across several rounds of fixes
      (a `<meta viewport>` tag, explicit top-anchored centering, pinning the
      capture's own pixel dimensions) that renderer never reliably matched
      what the capture already had. `buildPdf.ts`'s `buildPdfFromJpegPages`
      removes it entirely: it writes a minimal, valid PDF's bytes by hand —
      a Catalog, a Pages tree, and per page a Page object whose `/MediaBox`
      is exactly the capture's own size, an Image XObject holding the raw
      JPEG bytes untouched (`/Filter /DCTDecode` — no re-encoding), and a
      one-line content stream (`q W 0 0 H 0 0 cm /Im0 Do Q`) that scales that
      image to exactly fill the page. There is no layout engine left to
      disagree with the capture, because there is no layout engine at all.
- **Notification (Flow 7.1) is a real device feature**, per explicit product
  direction, not three cosmetic toggles. Daily Reminder and Monthly Report
  are real `expo-notifications` local triggers
  (`src/data/notificationSettings.ts`), and the master "Notifications"
  toggle reflects the OS's actual permission state rather than being an
  app-level preference of its own:
  - **The master toggle can't really be "toggled."** Its own Figma subtitle
    says outright that it mirrors Settings ("Enable notifications in
    Settings."), and no app can flip OS notification permission directly
    once the user has answered the first prompt. It reads
    `Notifications.getPermissionsAsync()`; tapping it calls
    `requestPermissionsAsync()` only the first time (`canAskAgain`), and
    otherwise opens the device's real Settings app
    (`Linking.openSettings()`) — there's nothing else a tap on it could
    honestly do.
  - **Turning on Daily Reminder or Monthly Report first ensures that same
    permission** (requesting it, or routing to Settings via an `Alert` if
    it's already been permanently denied) before anything is scheduled —
    a toggle that "succeeds" with nothing actually able to fire would be a
    silent lie.
  - **Daily Reminder's flow matches what the Monthly reminder-1 frame
    actually shows**, not just its label: the screenshot has the toggle
    already green *underneath* the open time-picker modal, so turning it on
    shows it as on immediately and opens `TimerPickerModal` in the same
    gesture, rather than waiting for Apply. Backing out of that *first*
    open without applying reverts the toggle — nothing was ever actually
    scheduled, so it shouldn't read as on. Re-opening the picker later from
    the already-set Timer row (to change the time) behaves differently on
    cancel: it just closes, since there a real reminder stays active either
    way. `NotificationScreen.tsx` tracks which case is which with one piece
    of state (`timerModalMode`) rather than trying to infer it from other
    state.
  - **Monthly Report's fixed notification time (9:00 AM) is the
    implementer's own reading of "the morning of the 1st"** — Figma's copy
    promises that but draws no picker for it, unlike Daily Reminder.
  - Figma's Modal/Timer literally titles itself "Select Month" — a
    copy-paste leftover from reusing the month-picker modal's own header
    component with the wrong text still in it, not a real second
    month-selection flow (the modal's content is unambiguously a time
    picker: hour, minute, AM/PM wheels). Read as an authoring mistake and
    corrected to "Select Time" in `TimerPickerModal.tsx`, the same way
    "alomost" was corrected on the Generating-PDF modal.
  - `expo-notifications`' remote push features aren't available in Expo Go
    on SDK 53+, but this project only ever uses its local scheduling API
